...
# --- Additional features: private messages, stats, profiles, notifications ---
import datetime

PRIVATE_CHAT_FILE = "private_chat.json"
PROFILES_FILE = "profiles.json"
NOTIFICATIONS_FILE = "notifications.json"

private_chats = load_json(PRIVATE_CHAT_FILE, [])
profiles = load_json(PROFILES_FILE, {})
notifications = load_json(NOTIFICATIONS_FILE, [])

@app.get("/private_chat")
def get_private_chat():
    u1 = request.args.get('u1'); u2 = request.args.get('u2')
    if not u1 or not u2: return jsonify([])
    conv = [m for m in private_chats if (m.get('from')==u1 and m.get('to')==u2) or (m.get('from')==u2 and m.get('to')==u1)]
    return jsonify(conv)

@app.post("/private_chat")
def post_private_chat():
    data = request.json or {}
    frm = data.get('from'); to = data.get('to'); text = data.get('text','').strip()
    if not frm or not to or not text: return jsonify({'error':'invalid'}),400
    msg = {'from':frm,'to':to,'text':text,'time': int(time.time())}
    private_chats.append(msg); save_json(PRIVATE_CHAT_FILE, private_chats)
    notifications.append({'type':'pm','user':to,'from':frm,'text':text,'time':int(time.time())})
...
def update_profile():
    data = request.json or {}
    login = data.get('login')
    if not login: return jsonify({'error':'login required'}),400
    prof = profiles.get(login,{})
    prof['name'] = data.get('name', prof.get('name',''))
    prof['bio'] = data.get('bio', prof.get('bio',''))
    prof['avatar'] = data.get('avatar', prof.get('avatar',''))
    profiles[login]=prof; save_json(PROFILES_FILE, profiles)
    return jsonify({'ok':True})
...
def get_notifications():
    ...
    user_notifs = [n for n in notifications if n.get('user')==user]
    return jsonify(user_notifs)

@app.post("/admin/notifications/clear")
def clear_notifications():
    data = request.json or {}
    if not check_admin_payload(data): return jsonify({'error':'admin auth required'}),403
    notifications.clear(); save_json(NOTIFICATIONS_FILE, notifications)
    return jsonify({'ok':True})
...
def get_stats():
    ...
    tests_list = load_json('tests.json', [])
    chat_list = load_json('chat.json', [])
    private_list = load_json(PRIVATE_CHAT_FILE, [])
    return jsonify({
        'users': len(users),
        'notes': len(notes_list),
        'tests': len(tests_list),
        'chat_messages': len(chat_list),
        'private_messages': len(private_list),
...
# --- Chat with Socket.IO real-time events ---
CHAT_FILE = "chat.json"
# load messages list (kept in memory for faster responses)
...
def post_chat():
    ...
    msg = {"user":user,"text":text,"time": int(time.time())}
    chat_messages.append(msg)
    save_json(CHAT_FILE, chat_messages)
...
# Socket.IO handler: clients can also send messages via socket
...
# --- Private chat with Socket.IO real-time ---
PRIVATE_CHAT_FILE = "private_chat.json"
private_chats = load_json(PRIVATE_CHAT_FILE, [])
...
def handle_send_private(data):
    ...
    if not sender or not recipient or not text:
        return
    msg = {'from': sender, 'to': recipient, 'text': text, 'time': int(time.time())}
    private_chats.append(msg)
    save_json(PRIVATE_CHAT_FILE, private_chats)
    emit('new_private_message', msg, room=recipient)
    emit('new_private_message', msg, room=sender)

@app.get("/private_chat")
def get_private_chat_http():
    u1 = request.args.get('u1'); u2 = request.args.get('u2')
    if not u1 or not u2:
        return jsonify([])
    conv = [m for m in private_chats if (m['from']==u1 and m['to']==u2) or (m['from']==u2 and m['to']==u1)]
    return jsonify(conv)

@app.post("/private_chat")
def post_private_chat_http():
    data = request.json or {}
    frm = data.get('from'); to = data.get('to'); text = (data.get('text') or '').strip()
    ...
    save_json(PRIVATE_CHAT_FILE, private_chats)
    try:
        socketio.emit('new_private_message', msg, room=to)
        socketio.emit('new_private_message', msg, room=frm)
    except Exception:
        pass
    return jsonify({'ok':True})
# --- end private chat Socket.IO ---
